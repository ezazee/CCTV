  ...
  <div class="row">
    <div class="col-xs-12 col-sm-12 col-md-12 col-lg-12">
      ...
      if ($level_user == 1) {
        ...
        echo '<button type="button" class="btn btn-success btn-flat access-failed" ' . ($row_user['is_create'] === '0' ? 'style="visibility:hidden"' : '') . '><i class="fa fa-plus"></i> Tambah Baru</button>';
      }
      echo '
          </div>
        </div>
      ...
      if ($result->num_rows > 0) {
        ...
        while ($row = $result->fetch_assoc()) {
          ...
          if ($level_user == 1) {
            echo '
                      <a href="#modalEdit" ' . ($row_user['is_update'] === '0' ? 'style="visibility:hidden"' : '') . 'class="btn btn-warning btn-xs enable-tooltip" title="Edit" data-toggle="modal"'; ?> onclick="getElementById('txtid').value='<?PHP echo $row['shift_id']; ?>';getElementById('txtname').value='<?PHP echo $row['shift_name']; ?>';getElementById('txtin').value='<?PHP echo $row['time_in']; ?>';getElementById('txtout').value='<?PHP echo $row['time_out']; ?>';"><i class="fa fa-pencil-square-o"></i> Ubah</a>
  <?php echo '
                    <buton data-id="' . epm_encode($row['shift_id']) . '" class="btn btn-xs btn-danger delete" ' . ($row_user['is_delete'] === '0' ? 'style="visibility:hidden"' : '') . ' title="Hapus"><i class="fa fa-trash-o"></i> Hapus</button>';
          } else {
            echo '
                      <button type="button" class="btn btn-warning btn-xs access-failed enable-tooltip" title="Edit" ' . ($row_user['is_update'] === '0' ? 'style="visibility:hidden"' : '') . '><i class="fa fa-pencil-square-o"></i> Ubah</button>
                      <buton type="button" class="btn btn-xs btn-danger access-failed" title="Hapus" ' . ($row_user['is_delete'] === '0' ? 'style="visibility:hidden"' : '') . '><i class="fa fa-trash-o"></i> Hapus</button>';
          }
          echo '
                  </div>
                  </td>
