    ...
    <div class="col-xs-12 col-sm-12 col-md-12 col-lg-12">
      ...
      if ($result->num_rows > 0) {
        ...
        while ($row = $result->fetch_assoc()) {
          ...
          if ($level_user == 1) {
            echo '
                    <a href="#modalEdit" ' . ($row_user['is_update'] === '0' ? 'style="visibility:hidden"' : '') . 'class="btn btn-warning btn-xs enable-tooltip" title="Edit" data-toggle="modal"'; ?> onclick="getElementById('txtid').value='<?PHP echo $row['position_id']; ?>';getElementById('txtnama').value='<?PHP echo $row['position_name']; ?>';"><i class="fa fa-pencil-square-o"></i> Ubah</a>
  <?php echo '
                <buton data-id="' . epm_encode($row['position_id']) . '" class="btn btn-xs btn-danger delete" title="Hapus" ' . ($row_user['is_delete'] === '0' ? 'style="visibility:hidden"' : '') . '><i class="fa fa-trash-o"></i> Hapus</button>';
          } else {
            echo '
