...
echo'
    <section class="container_box">
      <div class="row">';
      if(isset($_GET['from']) OR isset($_GET['to'])){
        echo'<h3 class="text-center">LAPORAN DETAIL HARIAN<br>PERIODE WAKTU '.bulan_indo2((int)$_GET['from']).' - '.$_GET['to'].'</h3>';}
        else{
        echo'<h3 class="text-center">LAPORAN DETAIL BULAN<br>'.bulan_indo2((int)$month).' - '.$year.'</h3>';
       }
        echo'
    ...
    $query_alpha="SELECT presence_id FROM presence WHERE $filter";
...
echo'
    <section class="container_box">
      <div class="row">';
      if(isset($_GET['from']) OR isset($_GET['to'])){
        echo'<h3>DATA ABSENSI BULAN '.bulan_indo2((int)$_GET['from']).' - '.$_GET['to'].'</h3>';}
        else{
        echo'<h3>DATA ABSENSI BULAN '.bulan_indo2((int)$month).' - '.$year.'</h3>';
      }
        echo'
        <p>NIP   : '.$row['employees_nip'].'</p>
        <p>Nama   : '.$row['employees_name'].'</p>
