  ...
  <div class="row">
    <div class="col-xs-12 col-sm-12 col-md-12 col-lg-12">
      ...
      if (!empty($_GET['id'])) {
        ...
        $query = "SELECT employees.id,employees.employees_nip,employees.employees_name,employees.position_id,position.position_name FROM employees,position WHERE employees.position_id=position.position_id AND employees.id='$id'";
        $result = $connection->query($query);
        if ($result->num_rows > 0) {
          $row = $result->fetch_assoc();
          echo '
        <h4 class="text-danger">NIP : ' . $row['employees_nip'] . '</h4>
        <h4>Nama   : ' . $row['employees_name'] . '</h4>
          ...
          <div class="modal-dialog modal-sm">
            <div class="modal-content">
              <div class="modal-header">
                <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                  <span aria-hidden="true">&times;</span></button>
                <h4 class="modal-title">Ubah Status Kehadiran</h4>
              </div>

              <form id="validate" action="javascript:void(0);" class="update-present form">
