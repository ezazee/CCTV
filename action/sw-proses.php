...
switch (@$_GET['action']) {
  case 'login':
    ...
    if (empty($error)) {
      ...
      $query_login = "SELECT id,employees_email,employees_name,is_active,created_cookies FROM employees WHERE employees_email='$email' AND employees_password='$password'";
      ...
      if ($result_login->num_rows > 0) {
        ...
        if ($row['is_active'] === '2') {
          ...
          if ($row['is_active'] === '1') {
            echo 'Akun anda belum di aktivasi';
          } else {
  ...
  case 'forgot':
    ...
    if (empty($error)) {
      $bcc    = 'agunghardiyanto12@gmail.com';
        ...
        if ($result->num_rows > 0) {
          ...
          if ($connection->query($update) === false) {
            ...
            try {
              ...
              $mail->addAddress($employees_email, '-');     //Add a recipient
  ...
  case 'absent':
    ...
    if (empty($error)) {
      ...
      $query_u = "SELECT employees.id,employees.employees_code,employees.employees_name,employees.shift_id,shift.shift_id,shift.time_in,shift.time_out FROM employees,shift WHERE employees.shift_id=shift.shift_id AND employees.id='$row_user[id]' AND employees.employees_code='$qrcode'";
  ...
  case 'absent-selfie':
    ...
    if (empty($error)) {
      // Cek User yang sudah login -----------------------------------------------
      $query_u = "SELECT employees.id,employees.employees_code,employees.employees_name,employees.shift_id,shift.shift_id,shift.time_in,shift.time_out FROM employees,shift WHERE employees.shift_id=shift.shift_id AND employees.id='$row_user[id]' AND employees.is_active=2";
      $result_u = $connection->query($query_u);
      if ($result_u->num_rows > 0) {
        $row_u = $result_u->fetch_assoc();
        $time_out     = strtotime('' . $row_u['time_out'] . ' - 30 minute');
        ...
        if ($result->num_rows > 0) {
          ...
          if ($time_out < $time) {
            if ($row['time_out'] == '00:00:00') {
              //Update Jam Pulang
              $update = "UPDATE presence SET time_out='$time',latitude_longtitude_out='$latitude', img_out='$image' WHERE employees_id='$row_u[id]' AND presence_date='$date'";
              if ($connection->query($update) === false) {
                die($connection->error . __LINE__);
                echo 'Sepetinya sitem kami sedang error!';
              } else {
    ...
    if ($result_cuty->num_rows > 0) {
      while ($row_cuty = $result_cuty->fetch_assoc()) {
        if ($row_cuty['cuty_status'] == '1') {
          $status = '<span class="badge badge-primary">Disetujui Operator</span>';
        } elseif ($row_cuty['cuty_status'] == '2') {
          $status = '<span class="badge badge-danger">Tidak disetujui Operator</span>';
        } elseif ($row_cuty['cuty_status'] == '4') {
          $status = '<span class="badge badge-success">Disetujui Admin</span>';
        } elseif ($row_cuty['cuty_status'] == '5') {
          $status = '<span class="badge badge-danger">Tidak disetujui Admin</span>';
        } else {
